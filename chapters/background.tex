...
\subsection{Stack}
The stack is a data structure organized according to a LIFO (Last In, First Out) policy and used primarily for managing function calls. Each function call involves the creation of a \emph{stack frame} containing local variables, parameters and return information (such as the return address, which tells the program the point in the calling code from which to resume execution once the function has been exited). The allocation and deallocation of memory on the stack are automatic and handled by the compiler: when a function terminates, its corresponding frame (figure \ref{fig:stack}) is removed.

\begin{figure}[h]
    \centering
    \includegraphics[scale=.22]{images/stack.png}
    \caption{Example of a stack frame structure.}
    \label{fig:stack}
\end{figure}
...
\subsection{Theory of refcount-based UAF exploitation}\label{sec:exploiting-uaf-refcount}
...
Concrete examples of this exploitation process can be found in vulnerabilities such as CVE-2021-0920 \cite{bib:exploit-cve-2021-0920} and CVE-2025-21756 \cite{bib:exploit-cve-2025-21756}, which illustrate how reference-counting and object-lifetime errors can be transformed into exploitable use-after-free conditions in real systems.
...
\begin{itemize}
    ...
    \item \textit{fuzzing tools}: generation of random inputs in order to test different execution paths for the analyzed software. An example is CountDown
...
In table \ref{table:tools} are some examples of tools capable of analyzing code and detecting use-after-free vulnerabilities. Since each vulnerability exploits different weaknesses, we have compiled a list of tools with different purposes and modes of operation to provide a broader range of options in this regard.

{
    \begin{table}[p]
        \small
        \centering
        \begin{tabular}{|S{c}|S{c}|S{c}|S{c}|S{c}|S{c}|}
            \hline
            \textbf{Tool} & \textbf{\makecell{LLVM\\based}} & \textbf{\makecell{Usable in\\kernel}} & \textbf{\makecell{Public\\source}} & \textbf{\makecell{Supports\\refcount}} & 
            \textbf{\makecell{Static/\\dynamic}}\\\hline
            \textbf{CountDown} \cite{bib:countdown} & Yes & Yes & Yes & Yes & Dynamic\\\hline
            \textbf{UAFX} \cite{bib:uafx} & Yes & Yes & Yes & No* & Static\\\hline
            Infer \cite{bib:infer} & Yes (+ Clang) & Yes & Yes & No & Static\\\hline
            \makecell{Clang Static\\Analyzer \cite{bib:clang}} & Yes & Yes & Yes & No & Static\\\hline
            Syzkaller \cite{bib:syzkaller} & Yes & Yes & Yes & No & Dynamic\\\hline
            GCC-fanalyzer \cite{bib:gcc} & No & No & Yes & No & Static \\\hline
            CRed \cite{bib:cred} & Yes & No & No & No & Dynamic\\\hline
            Cppcheck \cite{bib:cppcheck} & Yes & No & Yes & No & Static\\\hline
            \makecell{SVF-derived\\tools \cite{bib:svf}} & Yes & No & Yes & No & Static\\\hline
            FreeWill \cite{bib:freewill} & Yes & No & No & Yes & Dynamic\\\hline
            LinkRID \cite{bib:linkrid} & Yes & Yes & No & Yes & Hybrid\\\hline
            CRCount \cite{bib:crcount} & Yes & No & No & Yes & Dynamic\\\hline
            UAFChecker \cite{bib:uafchecker} & Yes & No & No & No & Static\\\hline
            DCUAF \cite{bib:dcuaf} & Yes & Yes & No & No & Static\\\hline
            SinkFinder \cite{bib:sinkfinder} & Yes & No & No & Yes & Static\\\hline
            Goshawk \cite{bib:goshawk} & Yes & Yes & Yes & No & Static \\\hline
            CID \cite{bib:cid} & Yes & Yes & No & Yes & Static \\\hline
            \multicolumn{6}{S{l}}{\makecell{* In theory, UAFX is capable of detecting reference counting issues as well;\\however, by design, it has been limited to algorithmically filter them out\\because their tests revealed too many false positives.}}
        \end{tabular}
        \caption{Complete list of tools that we have considered for this study.}
        \label{table:tools}
    \end{table}
}
...
Since our focus was on a use-after-free vulnerability involving a reference counting issue, we also excluded \emph{some} tools whose papers clearly stated that they would not be effective for this purpose.\\
...
\begin{enumerate}
    ...
    To identify the same object across different virtual machines during fuzzing, it uses a checksum of the call stack at the time of the object's initialization through \texttt{refcount\_set}.

    CountDown creates a RAI map (\texttt{refcount-address-ID}) to link the memory addresses holding refcount values with their refcount IDs:
    \begin{itemize}
        \item when a refcount is initialized, a new entry in the RAI map is created with its address and ID;
        \item when a refcount reaches zero, the entry is removed from the map;
        \item when a refcount is incremented or decremented, CountDown searches the address in the map to find its ID and records the operation.
    \end{itemize}

    During fuzzing, to connect refcount operations with syscalls, CountDown records the Syzkaller syscall number (NR) for each operation and considers that one syscall may modify a refcount many times, but it only cares about the final variation ($\delta$). In order to do so, every API listed in table \ref{table:refcounts} is modified to record every single change.
    ...
    \item \textbf{Reworking relationships between system calls}: \circled{1} CountDown constructs multiple relations among syscalls and refcounts starting from the operations mapped in the 4-tuple format $(S, O, \delta, V)$.
    
    First, it identifies syscalls that effectively change the refcount of any object. \circled{2} It projects a set of 4-tuples in a new map, where the key is the refcount ID and the value is a set of syscalls and their changes to the refcount ($\delta$), avoiding relations between syscalls from different programs (which can quickly become unrealiable).
    
    Based on the shared refcounts, every relation is reworked in a new one defined as \texttt{RefcntRelation}. The assumption is that, if two syscalls access the same refcount within one execution, they have a stronger relation with respect to triggering more complicated refcount operations. Each syscall set associated with a specific refcount is then split into a set of syscalls pairs \circled{3}. Following that, CountDown creates the syscall relation table, where each unique pair of syscalls has an entry and the value is the number of shared refcounts accessed by both \circled{4}.
    ...
    As explained in the paper, this newly constructed relation is more effective at guiding the fuzzer toward detecting refcount-related UAF bugs. However, the kernel code base is very large and refcount operations are not densely distributed. To solve the problem, the \texttt{RefCntRelation} is merged with the Syzkaller's standard \texttt{SyzRelation} and handled based on the fuzzing time. At the beginning of fuzzing, CountDown assigns a high priority to the original coverage-based relation to quickly explore more kernel code. As time goes on, it allocates more weight to CountDown's own relation so that the fuzzer can focus on testing complex refcount operations. This can be represented as the following formula:
