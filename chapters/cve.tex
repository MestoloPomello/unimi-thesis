...
In order to understand exactly why the bug occurs, we can start by looking at the patch the developers applied to fix it. Before the patch \cite{bib:kernel-patch} (kernel versions until 6.6.78 \cite{bib:af-vsock}), the vulnerable code in \texttt{af\_vsock} looked like this:
...
\noindent The maintainers added a simple \texttt{if} that checks if the socket is really "dead" before removing the binding. We can better understand how it works by working backward, starting with the call to the \texttt{vsock\_remove\_bound} function.
...
\noindent This means that, if the socket was not actually unbound, its reference counter was still decremented and the memory was freed, even though a valid reference to it still existed.
